...
class Report extends CI_Model
{
	...
	static function month_amount($year,$month){
        $total = 0;
        $query = "SELECT * FROM fx_payments WHERE MONTH(payment_date) = '$month' AND refunded = 'No' AND YEAR(payment_date) = '$year'";
    ...
    static function year_amount($year){
        $total = 0;
        $query = "SELECT * FROM fx_payments WHERE refunded = 'No' AND YEAR(payment_date) = '$year'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
    ...
    static function invoiced($year,$month){
    	$query = "SELECT * FROM fx_invoices WHERE MONTH(date_saved) = '$month' AND YEAR(date_saved) = '$year'";
    ...
    static function total_paid(){
        $total = 0;
        $query = "SELECT * FROM fx_payments WHERE refunded = 'No'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
