...
class Expense extends CI_Model
{
	...
	static function expenses_by_client($company, $report_by = NULL){
	 ...
	 static function by_range($start,$end,$report_by = NULL){
	 	switch ($report_by) {
	 		case 'billable':
	 			$sql = "SELECT * FROM fx_expenses WHERE billable = '1' AND expense_date BETWEEN '$start' AND '$end'";
	 			break;
	 		case 'unbillable':
	 			$sql = "SELECT * FROM fx_expenses WHERE billable = '0' AND expense_date BETWEEN '$start' AND '$end'";
	 			break;
	 		case 'billed':
	 			$sql = "SELECT * FROM fx_expenses WHERE invoiced = '1' AND expense_date BETWEEN '$start' AND '$end'";
	 			break;
	 		
	 		default:
	 			$sql = "SELECT * FROM fx_expenses WHERE expense_date BETWEEN '$start' AND '$end'";
	 			break;
	 	}
