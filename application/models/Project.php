...
class Project extends CI_Model
{
   ...
   /* static function pinned($type = 'projects')
    ...
    static function staff_work($duration,$user){
        ...
        switch ($duration) {
            case 'week':
                $tasklogs = "SELECT * FROM fx_tasks_timer WHERE user = '$user' AND YEARWEEK(`date_timed`,1) = YEARWEEK(CURDATE(),1)";
                $tasks = self::$db->query($tasklogs)->result();
                $projectlogs = "SELECT * FROM fx_project_timer WHERE user = $user AND YEARWEEK(`date_timed`,1) = YEARWEEK(CURDATE(),1)";
                $projects = self::$db->query($projectlogs)->result();
                break;
            case 'today':
                $tasklogs = "SELECT * FROM fx_tasks_timer WHERE user='$user' AND DATE(`date_timed`)=DATE(NOW())";
                $tasks = self::$db->query($tasklogs)->result();
                $projectlogs = "SELECT * FROM fx_project_timer WHERE user=$user AND DATE(`date_timed`)=DATE(NOW())";
                $projects = self::$db->query($projectlogs)->result();
                break;
            case 'month':
                $month = date('m');
                $year = date('Y');
                $tasklogs = "SELECT * FROM fx_tasks_timer WHERE user = '$user' AND MONTH(date_timed) = '$month' AND YEAR(date_timed) = '$year'";
                $tasks = self::$db->query($tasklogs)->result();
                $projectlogs = "SELECT * FROM fx_project_timer WHERE user = '$user' AND MONTH(date_timed) = '$month' AND YEAR(date_timed) = '$year'";
                $projects = self::$db->query($projectlogs)->result();
                break;
            case 'year':
                $year = date('Y');
                $tasklogs = "SELECT * FROM fx_tasks_timer WHERE user = '$user' AND YEAR(date_timed) = '$year'";
                $tasks = self::$db->query($tasklogs)->result();
                $projectlogs = "SELECT * FROM fx_project_timer WHERE user = '$user' AND YEAR(date_timed) = '$year'";
                $projects = self::$db->query($projectlogs)->result();
                break;
