...
class Rules_regulation extends MX_Controller {
    ...
    function index()
    {
				...
				$data['rules']          = $this->db->query("select * from fx_rules_regulation where status = 0 order by date_created  asc ")->result_array();
                $this->template
					 ->set_layout('users')
	...
	function add()
	{
		if ($this->input->post()) { 
			...
			$this->db->insert('fx_rules_regulation',$det);
	...
	function edit()
	{
		if ($this->input->post()) { 
			$det['content']   = $this->input->post('rule_description');
			$this->db->update('fx_rules_regulation',$det,array('id'=>$this->input->post('rule_tbl_id'))); 
    		redirect('rules_regulation');
		}else{
			$this->load->module('layouts');
			$this->load->library('template');
			...
			$data['page']           = 'rules';
			$data['role']           = $this->tank_auth->get_role_id();
			$data['rules_det']     = $this->db->query("select * from fx_rules_regulation where id = ".$this->uri->segment(3)."")->result_array();
