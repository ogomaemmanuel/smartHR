...
class Updates extends MX_Controller {
    ...
    function index() {
        ...
        $backups = is_dir('./assets/backup/') ? get_filenames('./assets/backup/') : array();
    ...
    function get_update($update = NULL) {
        ...
        if ($update) {
            ...
            $fp = fopen('./assets/updates/' . $update, 'w+');
    ...
    function download()
    {
        ...
        $path = "./assets/updates/update-".$build.".zip";
    ...
    function backup() {
        ...
        if (!is_writeable("./assets/backup/")) {
            Applib::make_flashdata(
                    array(
                        'response_status' => 'error',
                        'message' => 'We are unable to write to assets/backup folder'
            ));
            redirect($_SERVER['HTTP_REFERER']);
        }
        ...
        $this->zip->archive('./assets/backup/hrms_full_backup_' . date('Y-m-d') . '.zip');
        Applib::make_flashdata(
                array(
                    'response_status' => 'success',
                    'message' => 'Backup created and saved in assets/backup folder'
        ));
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    function extract()
    {
        ...
        $zip = new ZipArchive;
        if ($zip->open("./assets/updates/update-".$build.".zip") === TRUE) {
            $zip->extractTo('./');
            $zip->close();
            $data['json'] = array(
                "result"    => "1",
    ...
    function install() {
        ...
        $zip = new ZipArchive;
        if ($zip->open('./assets/updates/' . $latest_version . '.zip') === TRUE) {
            $zip->extractTo('./');
            $zip->close();
            // perform db changes
...
//        $this->db_upd_tables($src['data']);
    ...
    function mysql_backup() {
        ...
        if (!write_file('./assets/backup/database-full-backup_' . date('Y-m-d') . '.zip', $backup)) {
            $this->session->set_flashdata('response_status', 'error');
            $this->session->set_flashdata('message', 'Database backup failed cannot write to /assets/database.backup folder.');
            redirect('updates');
        }
        $this->db_json(array("data" => TRUE, "url" => './assets/backup/database-full-backup_' . date('Y-m-d') . '.json'));
        return true;
    }
