...
class Settings extends MX_Controller {
	...
	function leave_types(){
		if ($this->input->post()) {
			...
			if($tbl_id == ''){
 				$this->db->insert('fx_leave_types',$det);
			}else{ 
				$this->db->update('fx_leave_types',$det,array('id'=>$tbl_id));
			} 
	...
	function delete_leave_types(){
		if ($this->input->post()) {
			$det['status']   = 1;
		    $this->db->update('fx_leave_types',$det,array('id'=>$this->input->post('leave_type_id'))); 
			redirect('settings/?settings=leaves');
 		}else{
