<div class="modal-dialog">
	<div class="modal-content">
		...
		<button type="submit" class="btn btn-success"><?=lang('add_contact')?></button>
		</form>
		</div>
	</div>
	<!-- /.modal-content -->
	...
	$("#username").change(function(){
		...
		var msgbox = $("#status");
			if(username.length > 4){
				$("#status").html('<img src="<?=base_url()?>assets/images/reload.gif" />&nbsp;Checking username availability...');
				$.ajax({
				    type: "POST",
...
$("#email").change(function(){
	...
	var msgbox = $("#status");
		if(email.length > 5){
			$("#status").html('<img src="<?=base_url()?>assets/images/reload.gif" />&nbsp;Checking email availability...');
			$.ajax({
				type: "POST",
